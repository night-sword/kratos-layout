...
type WireApp func(arg []kratos.Option, bootstrap *conf.Bootstrap, confServer *conf.Server, confData *conf.Data, business *conf.Business) (*kratos.App, func(), error)
...
func Run(name *string, version string, wireApp WireApp) {
	...
	app, cleanup, err := wireApp(opts, bootstrap, bootstrap.Server, bootstrap.Data, bootstrap.Business)
	if err != nil {
		panic(err)
	}
	defer cleanup()
